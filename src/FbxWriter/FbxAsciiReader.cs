			...
			switch (c)
			{
				...
				default:
					...
					if (IsDigit(c, true)) // Number
					{
						...
						// FBX ASCII often uses scientific notation (e.g. 1e-5) without a decimal point.
						// Treat '.', 'e', and 'E' as real numbers.
						var dotIndex = str.IndexOf('.');
						var expIndex = str.IndexOfAny(new[] { 'e', 'E' });
						var isReal = dotIndex >= 0 || expIndex >= 0;
						if (isReal)
						{
							// Exponent notation: prefer double for safety.
							if (expIndex >= 0)
							{
								double d;
							...
							// Decimal without exponent: keep existing float-vs-double heuristic.
							var fracDigits = 0;
							if (dotIndex >= 0)
							{
								var fracEnd = (expIndex >= 0 ? expIndex : str.Length);
								fracDigits = Math.Max(0, fracEnd - dotIndex - 1);
							}
							if (fracDigits > 6)
							{
								double d;
